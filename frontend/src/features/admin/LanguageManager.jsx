...
const LanguageManager = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const normalizedName = formData.language_name.trim().toLowerCase();
    const isDuplicate = languages.some(
        lang => lang.language_name.toLowerCase().trim() === normalizedName && lang.id !== editingId
    );
    ...
    try {
      if (editingId) {
        ...
        setShowSuccessModal("Language updated successfully!");
      } else {
        await adminService.addLanguage(formData);
        setShowSuccessModal("Language added successfully!");
      }
      setFormData({ language_name: "", language_name_in_native: "" });
      setEditingId(null);
      fetchLanguages();
    } catch (err) {
  ...
  const handleDelete = async () => {
    ...
    try {
      await adminService.deleteLanguage(showDeleteModal);
      setShowSuccessModal("Language deleted successfully!");
      fetchLanguages();
    } catch (err) {
  ...
  const handleEdit = (lang) => {
    setFormData({
      ...
      language_name_in_native: lang.language_name_in_native || "",
    });
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <InputField
                ...
                value={formData.language_name_in_native}
                onChange={(e) =>
                  setFormData({
                    ...formData,
                    language_name_in_native: e.target.value,
                  })
                }
                dark={isDarkMode}
                placeholder="e.g. English"
                    ...
                    <tbody className="divide-y divide-gray-200/10">
                        ...
                        >
                            ...
                            <div className="flex justify-end gap-2">
                                ...
                                <button
                                onClick={() => setShowDeleteModal(lang.id)}
                                className={`p-2 rounded-lg transition-colors ${
                                    isDarkMode
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
            <div className={`w-full max-w-sm p-6 rounded-2xl shadow-2xl transform scale-100 animate-in zoom-in-95 duration-200 ${isDarkMode ? 'bg-gray-800 text-white' : 'bg-white text-gray-900'}`}>
                <div className="flex flex-col items-center text-center gap-4">
                    ...
                    <h3 className="text-lg font-bold">Duplicate Language</h3>
                    <p className={isDarkMode ? 'text-gray-300' : 'text-gray-600'}>
                        The language <strong>"{formData.language_name}"</strong> is already in the list.
                    </p>
                    <button 
